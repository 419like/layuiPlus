...
layui.define('jquery', function(exports){
  ...
  Element.prototype.tabAdd = function(filter, options){
    ...
    if(options.onClick){
    	titElem.append('<li lay-id="'+ (options.id||'') +'" onclick="'+ options.onClick +'(this);">'+ (options.title||'unnaming') +'</li>');
    }else{
  ...
  ,call = {
    ...
    ,tabAuto: function(){
      ...
      $('.layui-tab').each(function(){
        ...
        var span_zk = $(this).find('ul').find('span.layui-tab-bar');
        //响应式
        if(title.prop('scrollWidth') > title.outerWidth()+1 || span_zk.length > 0){
