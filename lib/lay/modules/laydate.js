...
layui.define(function(exports){
  ...
  Dates.scroll = function(type){
    ...
    //   if(type){
    //     return $(window.parent.document).scrollLeft();
    //   }else{
    //     return $(window.parent.document).scrollTop();
    //   }
    // }
  ...
  Dates.orien = function(obj, pos){
    ...
    if(rect.left+$(obj).width()>window.innerWidth){
      var tempW = $(obj).width()-rect.width;
    }else{
      var tempW = 0;
    }
    obj.style.left = rect.left + (pos ? 0 : Dates.scroll(1))-tempW + 'px';
    // if(rect.bottom + obj.offsetHeight/1.5 <= Dates.winarea()){
    if($(Dates.elem).offset().top-$(window.parent.document).scrollTop()<$(obj).height()){
      tops = rect.bottom - 1;
    } else {
